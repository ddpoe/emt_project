...
def test():
    ...
    print('matched #cell barcodes between CBC_GBC_summary and GSE data:',
          len(set(kazu_cbc2gbc.keys()) & set(gse_cbc_codes)))
    ...
    print('matched #cell barcodes between GSE and our processed data:',
          len(set(cbc_codes) & set(gse_cbc_codes)))

    old_cbc_gbc_frame = pd.read_csv(
        os.path.join(
            dir_path,
            'CBC_GBC_summary.txt'),
        delimiter='\t')
    ...
    print(old_cbcs[:5])
    print('matched #cell barcodes between two versions:',
          len(set(old_cbcs) & set(kazu_cbc2gbc.keys())))


def write_barcodes():
    dir_path = '../data/MCF10A_exp1/'
    kazu_onedrive_exp1_barcodes = pd.read_csv(
        os.path.join(
            dir_path,
            'barcodes.tsv'),
        header=None,
...
def test_kazu_new():
    dir_path = '../data/MCF10A_exp1/'
    exp1_cbc_gbc_frame = pd.read_csv(
        os.path.join(
            dir_path,
            'CBC_GBC_summary.txt'),
        delimiter='\t')
    exp2_cbc_gbc_frame = pd.read_csv(
        os.path.join(
            dir_path,
            'CBC_GBC.txt'),
        delimiter='\t')
    ...
    ann_obj = anndata.read_loom(
        os.path.join(
            dir_path,
            'possorted_genome_bam_RIG79.loom'))
    ...
    kazu_onedrive_exp1_barcodes = pd.read_csv(
        ...
        delimiter='\t')[0]
    kazu_onedrive_exp1_barcodes = [s[:-2] for s in kazu_onedrive_exp1_barcodes]
    whitelist_barcodes = pd.read_csv(
        os.path.join(
            dir_path,
            '10xv2_whitelist.txt'),
        header=None,
    ...
    print('matched #cell barcodes between kazu onedrive barcodes and adata:', len(
        set(adata_cbc_codes) & set(kazu_onedrive_exp1_barcodes)))
    print(
        'matched #cell barcodes between kazu onedrive barcodes and kazu onedrive CBC_GBC mapping:', len(
            set(kazu_onedrive_exp1_cbcs) & set(kazu_onedrive_exp1_barcodes)))

    print('matched #cell barcodes between whitelist barcodes and annData:',
          len(set(whitelist_barcodes) & set(adata_cbc_codes)))
    print(
        'matched #cell barcodes between whitelist barcodes and kazu onedrive barcodes:', len(
            set(whitelist_barcodes) & set(kazu_onedrive_exp1_barcodes)))
    print(
        'matched #cell barcodes between whitelist barcodes and kazu onedrive CBC_GBC mapping:', len(
            set(kazu_onedrive_exp1_cbcs) & set(whitelist_barcodes)))
