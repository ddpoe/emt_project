...
def main():
    ...
    def main_MAR(only_whole_data=False, use_pca=True):
        ...
        if config.use_dataset == 'pancreas':
            # due to package issue, somehow pancreas dataset has clusters not
            # capitalized
        ...
        adata.obs['whole_neighbor_MAR_is_eigenstable'] = np.zeros(
            len(adata), dtype=np.float)
        ...
        adata.obs['clusters_centroid_neighborhood_sample_mses'] = np.full(
            len(adata), -1)
        adata.obs['is_centroid_neighbor_indicator'] = np.zeros(
            len(adata), dtype=np.int)
        adata.obs['vel_norms'] = numpy.linalg.norm(
            adata.layers['velocity'], axis=1)
        ...
        adata.uns['whole_neighbor_MAR_models'] = [
            None for _ in range(len(adata))]
        ...
        df_gene_count_significance_eigenspace = pd.DataFrame(
        ...
        for label in label_set:
            ...
            if use_pca:
                ...
                pca_count_matrix = pca_model.transform(
                    raw_cluster_count_matrix)
            ...
            Neighbor MAR part
            '''
            mses, r2s, max_eigenval_reals, feature_norms, bias_norms, jacobs, MAR_models = neighbor_MAR(
                label_count_matrix, label_velocities, neighbor_num=config.MAR_neighbor_num, pca_model=pca_model)
            cluster_centroid_sample_mses, is_centroid_neighbor_indicator, closest_sample_id_in_indices = centroid_neighbor_MAR(
                label_count_matrix, label_velocities, neighbor_num=2000, pca_model=pca_model)
            ...
            if label != whole_data_label:
                ...
                adata.obs['clusters_centroid_neighborhood_sample_mses'][indices[is_centroid_neighbor_indicator]
                                                                        ] = cluster_centroid_sample_mses
                ...
                adata.obs['whole_neighbor_MAR_is_eigenstable'][indices[np.array(
                    max_eigenval_reals) <= 0]] = 1
                adata.obs['whole_neighbor_avg_feature_norms'] = feature_norms
                adata.obs['whole_neighbor_bias_norms'] = bias_norms
                adata.obs['whole_data_centroid_sample_errors'][indices[is_centroid_neighbor_indicator]
                                                               ] = cluster_centroid_sample_mses
